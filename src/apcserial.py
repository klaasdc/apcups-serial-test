...
#                     print('battery_replacetest_status_raw is now ' + str(battery_replacetest_status))
                ...
                self.ups_status['status_chg_cause_raw'] = status_chg_cause_raw
                #These are documented in the APC Modbus documentation
                if status_chg_cause_raw == 0:
                    self.ups_status['status_chg_cause'] = "SystemInitialization"
                if status_chg_cause_raw == 1:
                    self.ups_status['status_chg_cause'] = "HighInputVoltage"
                if status_chg_cause_raw == 2:
                    self.ups_status['status_chg_cause'] = "LowInputVoltage"
                if status_chg_cause_raw == 3:
                    self.ups_status['status_chg_cause'] = "DistortedInput"
                if status_chg_cause_raw == 4:
                    self.ups_status['status_chg_cause'] = "RapidChangeOfInputVoltage"
                if status_chg_cause_raw == 5:
                    self.ups_status['status_chg_cause'] = "HighInputFrequency"
                if status_chg_cause_raw == 6:
                    self.ups_status['status_chg_cause'] = "LowInputFrequency"
                if status_chg_cause_raw == 7:
                    self.ups_status['status_chg_cause'] = "FreqAndOrPhaseDifference"
                if status_chg_cause_raw == 8:
                    self.ups_status['status_chg_cause'] = "AcceptableInput"
                if status_chg_cause_raw == 9:
                    self.ups_status['status_chg_cause'] = "AutomaticTest"
                if status_chg_cause_raw == 10:
                    self.ups_status['status_chg_cause'] = "TestEnded"
                if status_chg_cause_raw == 11:
                    self.ups_status['status_chg_cause'] = "LocalUICommand"
                if status_chg_cause_raw == 12:
                    self.ups_status['status_chg_cause'] = "ProtocolCommand"
                if status_chg_cause_raw == 13:
                    self.ups_status['status_chg_cause'] = "LowBatteryVoltage"
                if status_chg_cause_raw == 14:
                    self.ups_status['status_chg_cause'] = "GeneralError"
                if status_chg_cause_raw == 15:
                    self.ups_status['status_chg_cause'] = "PowerSystemError"
                if status_chg_cause_raw == 16:
                    self.ups_status['status_chg_cause'] = "BatterySystemError"
                if status_chg_cause_raw == 17:
                    self.ups_status['status_chg_cause'] = "ErrorCleared"
                if status_chg_cause_raw == 18:
                    self.ups_status['status_chg_cause'] = "AutomaticRestart"
                if status_chg_cause_raw == 19:
                    self.ups_status['status_chg_cause'] = "DistortedInverterOutput"
                if status_chg_cause_raw == 20:
                    self.ups_status['status_chg_cause'] = "InverterOutputAcceptable"
                if status_chg_cause_raw == 21:
                    self.ups_status['status_chg_cause'] = "EPOInterface"
                if status_chg_cause_raw == 22:
                    self.ups_status['status_chg_cause'] = "InputPhaseDeltaOutOfRange"
                if status_chg_cause_raw == 23:
                    self.ups_status['status_chg_cause'] = "InputNeutralNotConnected"
                if status_chg_cause_raw == 24:
                    self.ups_status['status_chg_cause'] = "ATSTransfer"
                if status_chg_cause_raw == 25:
                    self.ups_status['status_chg_cause'] = "ConfigurationChange"
                if status_chg_cause_raw == 26:
                    self.ups_status['status_chg_cause'] = "AlertAsserted"
                if status_chg_cause_raw == 27:
                    self.ups_status['status_chg_cause'] = "AlertCleared"
                if status_chg_cause_raw == 28:
                    self.ups_status['status_chg_cause'] = "PlugRatingExceeded"
                if status_chg_cause_raw == 29:
                    self.ups_status['status_chg_cause'] = "OutletGroupStateChange"
                if status_chg_cause_raw == 30:
                    self.ups_status['status_chg_cause'] = "FailureBypassExpired"
...
if __name__ == '__main__':
    ser = serial.Serial('COM5', 9600, timeout=APC_RCV_TIMEOUT, parity=serial.PARITY_NONE)
    print(ser.name)
